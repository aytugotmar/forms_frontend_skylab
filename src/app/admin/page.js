...
import FormPerformanceChart from "./components/analytics/FormPerformanceChart";
...
function StatCard({
  ...
  const content = (
    <div className="rounded-2xl border border-black/40 bg-black/15 p-5 shadow-sm backdrop-blur transition-colors hover:bg-black/20">
      <div className="flex items-start gap-3">
        ...
        {href ? <ArrowRight className="h-4 w-4 text-neutral-500" /> : null}
      </div>
    </div>
  );
  ...
  return (
    <Link
      href={href}
      className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/20 rounded-2xl"
    >
      {content}
    </Link>
  );
...
export default function Admin() {
  ...
  const stats = useMemo(() => {
    ...
    const totalResponses = forms.reduce(
      (acc, form) => acc + (Number(form?.responseCount) || 0),
      0,
    );
    const linkedForms = forms.reduce(
      (acc, form) => acc + (form?.linkedFormId ? 1 : 0),
      0,
    );
  ...
  const topRespondedForms = useMemo(() => {
    return [...forms]
      .sort(
        (a, b) =>
          (Number(b?.responseCount) || 0) - (Number(a?.responseCount) || 0),
      )
      .slice(0, 5);
  }, [forms]);
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          href="/admin/forms"
          sparkline={sparklineData.totalForms}
        ...
        <StatCard
          ...
          hint="Tüm formların toplamı"
          href="/admin/forms"
          sparkline={sparklineData.totalResponses}
          sparklineColor="#8b5cf6"
        />
        <StatCard
          icon={Waypoints}
          label="Bağlı form"
          value={stats.linkedForms}
          hint="Başka bir forma yönlendiren"
          href="/admin/forms"
          sparkline={sparklineData.linkedForms}
        ...
        <StatCard
          ...
          hint="Form güncelleme zamanı"
          href="/admin/forms"
        />
      </div>

      {/* Analytics Section - Her zaman görünür */}
      <div className="space-y-6">
        {/* Haftalık Karşılaştırma */}
        <ComparisonCard data={comparisonData} />

        {/* Ana Grafikler - 2 Kolon */}
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <ResponseTrendsChart data={responsesTrendData} />
          <FormPerformanceChart data={formPerformanceData} />
        </div>

        {/* İkinci Sıra Grafikler - 2 Kolon */}
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <ResponseDistributionChart data={responseDistributionData} />
          <CompletionRatesChart data={completionRatesData} />
        </div>

        {/* Aktivite Heatmap - Tam Genişlik */}
        <ActivityHeatmap data={activityHeatmapData} />
      </div>

      {/* Form Listeleri - Conditional */}
      {isLoading ? (
        <div className="space-y-3">
          <div className="rounded-2xl border border-neutral-900 bg-neutral-950/50 p-4 text-sm text-neutral-400">
            Form listeleri yükleniyor...
          </div>
          <ListItemSkeleton count={3} />
        </div>
      ) : hasError ? (
        <div className="rounded-2xl border border-neutral-900 bg-neutral-950/50 p-6 text-sm text-neutral-400">
          Form listeleri yüklenemedi.
        </div>
      ) : forms.length === 0 ? (
        <div className="rounded-2xl border border-neutral-900 bg-neutral-950/50 p-6 text-sm text-neutral-400">
          ...
          <section className="space-y-3">
            ...
            <div className="space-y-1.5">
              {recentForms.map((form) => {
                const linkedForm = form?.linkedFormId
                  ? formsById.get(form.linkedFormId)
                  : null;
                return (
                  <ListItem
                    key={form.id}
